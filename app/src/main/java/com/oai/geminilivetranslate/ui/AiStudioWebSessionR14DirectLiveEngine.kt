...
(function(){
  ...
  const state={
    ...
    screenHeartbeatPendingText:'',
    screenTurnInFlight:false,
    screenHeartbeatsQueued:0,
    screenHeartbeatsCoalesced:0,
    screenHeartbeatOverrides:0,
  ...
  function rewriteEnvelope(body){
    ...
    for(let i=0;i<names.length;i++){
      ...
      if(state.screenHeartbeatEnabled&&state.screenSetupComplete&&state.screenHeartbeatPending&&!state.screenTurnInFlight&&!heartbeatInjected){
        ...
        const heartbeatText=String(state.screenHeartbeatPendingText||state.screenHeartbeatText||'').trim();
        if(realtime&&heartbeatText){
          try{
            ...
            state.screenHeartbeatPendingText='';
            ...
            emit('SCREEN_HEARTBEAT_TURN_BOUNDARY',{
              ...
              textChars:heartbeatText.length,
  ...
  function queueScreenHeartbeat(textOverride){
    const override=String(textOverride||'').trim().slice(0,4000);
    if(!state.screenHeartbeatEnabled||(!state.screenHeartbeatText&&!override)||!state.screenSetupComplete)return {ok:false,enabled:state.screenHeartbeatEnabled,setupComplete:state.screenSetupComplete};
    const now=Date.now();
    const inFlightAgeMs=state.screenTurnInFlight&&state.lastScreenHeartbeatAt?now-state.lastScreenHeartbeatAt:-1;
    if(state.screenHeartbeatPending){
      state.screenHeartbeatsCoalesced++;
      if(override&&!state.screenHeartbeatPendingText){
        state.screenHeartbeatPendingText=override;state.screenHeartbeatOverrides++;
      }
      if(state.screenHeartbeatsCoalesced===1||state.screenHeartbeatsCoalesced%20===0)emit('SCREEN_HEARTBEAT_COALESCED',{count:state.screenHeartbeatsCoalesced,inFlight:state.screenTurnInFlight,pendingTextChars:state.screenHeartbeatPendingText.length,queued:state.screenHeartbeatsQueued,injects:state.screenHeartbeatsInjected});
      return {ok:true,pending:true,coalesced:true,inFlight:state.screenTurnInFlight,inFlightAgeMs:inFlightAgeMs,queued:state.screenHeartbeatsQueued,coalescedCount:state.screenHeartbeatsCoalesced,injects:state.screenHeartbeatsInjected};
    }
    state.screenHeartbeatPending=true;state.screenHeartbeatPendingText=override;state.screenHeartbeatsQueued++;
    if(override)state.screenHeartbeatOverrides++;
    ...
    emit('SCREEN_HEARTBEAT_QUEUED',{count:state.screenHeartbeatsQueued,inFlight:state.screenTurnInFlight,inFlightAgeMs:inFlightAgeMs,override:!!override,textChars:(override||state.screenHeartbeatText).length});
    return {ok:true,pending:true,coalesced:false,inFlight:state.screenTurnInFlight,inFlightAgeMs:inFlightAgeMs,queued:state.screenHeartbeatsQueued,coalescedCount:state.screenHeartbeatsCoalesced,injects:state.screenHeartbeatsInjected};
  }
