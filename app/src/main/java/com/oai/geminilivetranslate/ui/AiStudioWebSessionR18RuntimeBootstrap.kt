 ...
 * reason about the runtime service without hard-coding bundle symbols.
 */
...
(function(){
  ...
  function isUiObject(value){
    ...
    try{
      if(typeof EventTarget==='function'&&value instanceof EventTarget){
        const n=typeName(value);
        if(/Window|Document|Element|Node|ShadowRoot/.test(n))return true;
      }
    }catch(_){}
    return false;
  }
  ...
  function hasDirectEvidence(sig){
    const s=sig&&sig.signals||[];
    if(s.indexOf('targetModel')>=0||s.indexOf('bidiGenerateContent')>=0||s.indexOf('getUserMedia')>=0)return true;
    return Number(sig&&sig.score||0)>=18&&s.length>=2;
  }
    ...
    try{
      ...
      for(let si=0;si<sources.length;si++){
        ...
        for(let i=0;i<names.length;i++){
          ...
          const sig=sourceSignals(src);if(sig.uiCoupled||!hasDirectEvidence(sig))continue;
          const score=sig.score+profile.score;
          if(score<24)continue;
          out.push({receiver:obj,fn:fn,path:(item.prefix+'.'+safeSeg(name)).slice(0,280),name:safeSeg(name),arity:0,score:score,directSignals:sig.signals.slice(0,8),receiverSignals:profile.signals,sourceHash:hashText(src)});
        }
      }
    }catch(_){}
    return out;
  ...
  function discover(){
    ...
    state.strongCandidateCount=candidates.filter(function(c){return c.score>=30&&c.directSignals.length>=1&&c.arity===0;}).length;
  ...
  function structuredFrames(){
    ...
    try{
      ...
      for(let i=0;i<stack.length&&out.length<MAX_LEARNED;i++){
        ...
        const meta={index:i,fileHost:'',filePath:'',line:line,column:column,functionName:fnName,methodName:methodName,receiverType:typeName(receiver),arity:Number(fn.length||0),sourceHash:hashText(src),directSignals:sig.signals.slice(0,8),argsAvailable:args.available,argTypes:args.types,hasUiArg:args.hasUiArg};
  ...
  function installStructuredObserver(){
    try{
      ...
      X.prototype.send=function(body){
        try{const raw=String(this.__aisR183BRawUrl||''),text=typeof body==='string'?body:'';if(raw.indexOf('/v1/bidiGenerateContent')>=0&&text.toLowerCase().indexOf(TARGET_MODEL)>=0&&!/audio\/pcm/i.test(text))learnFromSetup(raw);}catch(_){}
  ...
  function invokeUnique(candidates){
    const strong=candidates.filter(function(c){return c.score>=30&&c.directSignals.length>=1&&c.arity===0;});
    ...
    try{
      ...
      setTimeout(checkProgress,120);setTimeout(checkProgress,700);setTimeout(checkProgress,2200);setTimeout(checkProgress,6000);return true;
  ...
  function tryScan(){
    ...
    const candidates=discover();if(invokeUnique(candidates))return;state.retryCount++;
    if(state.retryCount<state.maxRetries){state.stage='waiting-runtime';retryTimer=setTimeout(tryScan,900);}else{state.stage=state.stage==='ambiguous-candidates'?'ambiguous-candidates':'no-runtime-candidate';bridge('FINAL_NO_START',{stage:state.stage,retryCount:state.retryCount,top:state.top,learnedFrames:state.learnedFrames});}
  }
  ...
  function start(code){
    ...
    const c=liveCounters();state.bidiBefore=c.bidi;state.bidiAfter=c.bidi;state.setupBefore=c.setup;state.setupAfter=c.setup;state.startAt=Date.now();state.retryCount=0;state.selected=null;state.lastError='';state.stage='discovering-runtime';
    bridge('START',{targetLanguageCode:state.targetLanguage,bidiBefore:state.bidiBefore,setupBefore:state.setupBefore});setTimeout(tryScan,0);return describe();
  }
  function reset(){
    try{if(retryTimer)clearTimeout(retryTimer);}catch(_){}retryTimer=0;state.stage='idle';state.lastError='';state.scans=0;state.objectsVisited=0;state.functionsVisited=0;state.candidateCount=0;state.strongCandidateCount=0;state.invokeAttempts=0;state.invokeReturns=0;state.invokePromises=0;state.invokeErrors=0;state.bidiBefore=0;state.bidiAfter=0;state.setupBefore=0;state.setupAfter=0;state.startAt=0;state.lastScanAt=0;state.selected=null;state.top=[];state.retryCount=0;return describe();
  }
  function describe(){
    checkProgress();
