...
class AiStudioWebSessionR10SourceTest {
    ...
    @Test
    fun r121ExecutorKeepsDirectEngineAndUsesProgressAwareCompletion() {
        ...
        assertTrue(executor.contains("AiStudioWebSessionDirectEngine.DOCUMENT_START"))
        assertTrue(executor.contains("tryDirectEngineRecovery"))
        assertTrue(executor.contains("R12_DIRECT_SUBMIT_SUCCESS"))
        assertTrue(executor.contains("R12_TIMEOUT_POLICY"))
        ...
        assertTrue(executor.contains("kind == \"GENERATE_RESULT\""))
        assertTrue(executor.contains("parsedResult.copy(complete = true)"))
        assertTrue(executor.contains("phase == \"reset-after-stream\""))
        assertTrue(executor.contains("AiStudioGoogleAccountBootstrap.consumeStartUrl"))
        assertFalse(executor.contains("dispatchTouchEvent"))
        assertFalse(executor.contains("MotionEvent"))
        assertFalse(executor.contains("InputDevice.SOURCE_TOUCHSCREEN"))
